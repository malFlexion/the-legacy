...
5. **Card resolution + bracket wrapping** — response is scanned for card names; `auto_bracket_cards` wraps them in `[[Name]]` markup, re-stashing each newly-wrapped match into the placeholder list so substrings inside long names (`Void` inside `Chalice of the Void`) can't match inside a wrap we just made. Frontend renders those as `<a href=scryfall_uri target=_blank>` anchors (so they click through) and populates the left-panel card grid in the order they appear in the response.
...
### `/analyze-deck` uses the same pipeline

The Import & Analyze tab's "Analyze" button hits `/analyze-deck`, which runs the full chat-tab pipeline against the pasted decklist: `extract_query_cards` is applied per decklist line to collect up to 12 cards of ground-truth injection, RAG retrieves strategy/rules chunks only (`exclude_cards=True`), a strict analysis-instructions system message ("work ONLY from the cards in the pasted list — do not invent Leyline / Eldrazi / Chalice unless they're literally listed") is prepended before the user turn, and the response goes through `auto_bracket_cards` + `resolve_cards` the same way `/chat` does. Before this hardening the endpoint accepted a RAG context and a loose post-user system message, and the 1B model regularly confabulated entirely different decks.
...
Other Modelfile params: `num_predict 256` (chat responses run short to stay on-topic), `num_ctx 8192` (required: up to 3 card sheets + 10 RAG chunks + system prompt + history is easily 5–9k tokens; the earlier 2048 limit silently truncated the injected context mid-request). The server-side httpx timeout to Ollama is 300s — `performance-8x` CPU runs ~8 tok/s, so a full 512-token analyze response with an 8k prompt can legitimately take 2+ minutes; the earlier 120s ceiling was tuned for the simpler chat pipeline and caused `/analyze-deck` to 504 once card injection expanded the prompt. Callers can override `temperature` on any request. Sampling is pure top-k/top-p (Ollama defaults) — no beam search, no constrained decoding. Card validity is enforced *after* generation via the deterministic `card_index.resolve` pass, not by restricting the decoder.
...
- **Card Data**: Scryfall bulk data indexed locally (~36,662 cards, ~30,499 Legacy-legal). Dedup preference: English only, earliest `released_at` first, tie-broken by lowest `collector_number` — so iconic sets win, and within a set the main printing beats showcase / borderless / etched variants (OTJ `#15` High Noon beats `#314`, etc.). Fuzzy + word-boundary matching via rapidfuzz
